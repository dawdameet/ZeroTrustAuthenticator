...
#         # Compute similarity using Euclidean distance (adjustable threshold)
#         keystroke_diff = euclidean(
#             [k["time"] for k in past_keystrokes] if past_keystrokes else [0],
#             [k["time"] for k in current_keystrokes] if current_keystrokes else [0],
#         ) if past_keystrokes and current_keystrokes else float('inf')

#         mouse_diff = euclidean(
...
#         ) if past_mouse_movements and current_mouse_movements else float('inf')

#         threshold = 5000  # Adjust this value based on behavior patterns
#         if keystroke_diff < threshold and mouse_diff < threshold:
#             auth_status = "Authenticated"
#         else:
#             auth_status = "Rejected"

#         return jsonify({"message": auth_status, "keystroke_diff": keystroke_diff, "mouse_diff": mouse_diff}), 200
...
#     return jsonify({"message": "First-time behavior recorded"}), 200
@app.route('/behavior', methods=['POST'])
def receive_behavior():
    data = request.json
    user_id = "user123"  # Replace with real authentication later
